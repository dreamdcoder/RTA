...
def preprocessing(df):
    ...
    mode_df=pd.read_csv('../RTA/Model/mode.csv')
    ...
    # st.write(df)
    enc = joblib.load("../RTA/Model/X_Encoder.pkl")
    X = enc.transform(df)
    return X

def prediction(df):
    model = joblib.load("../RTA/Model/model.pkl")
    y_enc=joblib.load(("../RTA/Model/Y_Encoder.pkl"))
    y=model.predict(df)
